    ...
    for k in range(2):
        for j in range(4):
            verso = modeloPoemas.make_sentence(
                init_state=None, tries=2000, max_words=7, state_size=2, test_output=False).strip().capitalize()
            lV = verso.split()
            for i in range(len(lV)):
                try:
                    if len(lV[i]) <= 1 and len(lV[i+1]) <= 1:
                        verso = modeloPoemas.make_sentence(
                            init_state=None, tries=2000, max_words=15, state_size=2, test_output=False).strip().capitalize()
                except:
                    pass
            poema += verso + '\n'
        if (k != 1):
...
run()
# agenda a tarefa em horários aleatórios entre 1 e 10 hrs
